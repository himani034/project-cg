...
def safe_insert(collection, data):
    try:
        ...
        print("MongoDB logging skipped:", e)
...
def sales_overview():
    try:
        ...
        logger.error(f"Sales overview failed: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
def demand_prediction(request: DemandRequest):
    try:
        ...
        safe_insert(prediction_collection, {
            "type": "demand_forecast",
            "input": input_data,
            "predicted_units_sold": prediction,
            "created_at": datetime.utcnow()
...
def document_search(request: SearchRequest):
    try:
        ...
        safe_insert(document_collection, {
            "query": request.query,
            "created_at": datetime.utcnow()
        })
